...
#   - US-DONE <id> when agent outputs <ralph>US-DONE US-XXX</ralph>
#
# Writes to .ralph/:
  ...
  case "$type" in
    ...
    "assistant")
      ...
      if [[ -n "$text" ]]; then
        ...
        # Check for US-DONE sigil (e.g. <ralph>US-DONE US-001</ralph>)
        if [[ "$text" =~ \<ralph\>US-DONE[[:space:]]+([^[:space:]<]+)\</ralph\> ]]; then
          local us_id="${BASH_REMATCH[1]}"
          log_activity "✅ Agent signaled US-DONE $us_id"
          echo "US-DONE $us_id" 2>/dev/null || true
        fi
